...
@router.post("/image") #, response_model=List[PredictionResponseModel])
def submit_image(
    ...
    toxic_predict = predict_text(text, settings.SEGMENT_CHUNK_SIZE, settings.TEXT_ENDPOINT)
    moderate = moderate_image(
        file.filename, settings.S3_BUCKET_NAME, settings.VIDEO_REKOGNITION_TIMEOUT
    )
    return toxic_predict,moderate
